...
"""class MultiModalPromptLearner(nn.Module):  ##Original
    def __init__(self, cfg, classnames, clip_model):
        super().__init__()
        n_cls = len(classnames)
        n_ctx = cfg.TRAINER.MAPLE.N_CTX
...
"""class MultiModalPromptLearner(nn.Module):
    ...
    def forward(self):
        ...
        visual_deep_prompts = []
        for index, layer in enumerate(self.compound_prompt_projections):
            visual_prompt = layer(self.compound_prompts_text[index])
            visual_deep_prompts.append(visual_prompt)
            self.print_info(visual_prompt, f"visual_deep_prompts[{index}]")

        projected_ctx = self.proj(self.ctx)
        self.print_info(projected_ctx, "projected_ctx")
...
"""class CustomCLIP(nn.Module):
    ...
    def forward(self, image, label=None):
        ...
        return logits"""
"""class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):       #original custom clip code
        super().__init__()
        self.prompt_learner = MultiModalPromptLearner(cfg, classnames, clip_model)
        self.tokenized_prompts = self.prompt_learner.tokenized_prompts
        self.image_encoder = clip_model.visual
    ...
    def forward(self, image, label=None):
        ...
        prompts, shared_ctx, deep_compound_prompts_text, deep_compound_prompts_vision = self.prompt_learner()
       # self.print_info(prompts, "prompts")
        #self.print_info(shared_ctx, "shared_ctx")
       # self.print_info(deep_compound_prompts_text, "deep_compound_prompts_text")
        #self.print_info(deep_compound_prompts_vision, "deep_compound_prompts_vision")

        text_features = self.text_encoder(prompts, tokenized_prompts, deep_compound_prompts_text)
       # self.print_info(text_features, "text_features")

        image_features = self.image_encoder(image.type(self.dtype), shared_ctx, deep_compound_prompts_vision)
       # self.print_info(image_features, "image_features")

        image_features = image_features / image_features.norm(dim=-1, keepdim=True)
       # self.print_info(image_features, "normalized_image_features")

        text_features = text_features / text_features.norm(dim=-1, keepdim=True)
        #self.print_info(text_features, "normalized_text_features")

        logits = logit_scale * image_features @ text_features.t()
       # self.print_info(logits, "logits")
    ...
    def forward(self, image, label=None, caption=None):
        ...
        if self.prompt_learner.training:
            if label is not None and isinstance(label, torch.Tensor) and label.dtype == torch.float:
                # Use KLDivLoss for mix-up soft labels
                log_probs = F.log_softmax(logits, dim=1)
                target_probs = label
                #(log_probs, "log_probs")
                #self.print_info(target_probs, "target_probs")
                #self.print_info(logits, "logits")
                #self.print_info(label, "label")
                loss = F.kl_div(log_probs, target_probs, reduction='batchmean')
            else:
               # print("else executed")
               # print(f"Labels: {label}")
               # print(f"Unique labels: {label.unique()}")
               # print(f"Logits shape: {logits.shape}")
                # Use cross_entropy for standard hard labels
                loss = F.cross_entropy(logits, label)
