...
class ResidualAttentionBlock(nn.Module):
    ...
    def freeze_all_layers_except_norm(self):
      #  print("Unfreezing layer norm of the residual attention block")
    ...
    def forward(self, x: torch.Tensor):
      #  print("INSIDE FORWARD FUNCTION OF RESIDUAL ATTENTION BLOCK")
...
class ResidualAttentionBlock_MaPLe(nn.Module):
    ...
    def freeze_all_layers_except_norm(self):
        for name, param in self.named_parameters():
         #   print("Unfreezing layer norm of the residual attention block maple") 
            if "ln_1" in name or "ln_2" in name:
                param.requires_grad = True
            else:
                param.requires_grad = False
    ...
    def forward(self, inputs):
     ...
     #   print("INSIDE FORWARD FUNCTION OF RESIDUAL ATTENTION BLOCK MAPLE")
        x = inputs[0]
...
class VisionTransformer(nn.Module):
    ...
    def freeze_all_layers_except_norm(self):
       # print("Unfreezing layer norm of the vision transformer")
        for name, param in self.named_parameters():
    ...
    def forward(self, x: torch.Tensor):
       # print("INSIDE Function VisionTransformer")
...
class AttentionPooling(nn.Module):
    def __init__(self, embedding_dim):
        ...
        # Learnable attention weights (vector of size embedding_dim)
        self.attention_weights = nn.Parameter(torch.randn(embedding_dim)).to("cuda")
        self.attention_weights = self.attention_weights.half()

    def forward(self, embeddings):
        # embeddings shape: [batch_size, num_tokens, embedding_dim]
        # Step 1: Compute scores by dot product between embeddings and attention_weights
        scores = torch.matmul(embeddings, self.attention_weights)  # Shape: [batch_size, num_tokens]
        
        # Step 2: Apply softmax to obtain attention probabilities
        attention_probs = F.softmax(scores, dim=1)  # Shape: [batch_size, num_tokens]

        # Step 3: Multiply attention probabilities with embeddings
        attention_probs = attention_probs.unsqueeze(-1)  # Shape: [batch_size, num_tokens, 1]
        pooled_embeddings = torch.sum(embeddings * attention_probs, dim=1)  # Shape: [batch_size, embedding_dim]
...
class VisionTransformer_MaPLe(nn.Module):
    ...
    def forward(self, x: torch.Tensor, shared_ctx, compound_deeper_prompts, clip_embeddings=None):
        #print("INSIDE FORWARD FUNCTION OF VISION TRANSFORMER MAPLE printing x as it is inputted to the forward function", x.shape)
        #test_input = torch.randn(4, 3, 224, 224)
        #x=self.dino.forward(x)  #chabged x_dino to x
        #print("shape of x_dino", x_dino.shape)  
        x = self.conv1(x)  # shape = [*, width, grid, grid]
       ...
       # x = torch.cat([x, x_dino], dim=1)  # Concatenates along the patch dimension
        #print("shape of x after concatenating with x_dino", x.shape)
        x = torch.cat(
            [self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device),
             x], dim=1)  # shape = [*, grid ** 2 + 1, width]
       # print("shape of x after concatenation", x.shape)
        #x_positional_embedding = torch.cat([self.positional_embedding, self.positional_embedding[:196]], dim=0) # for vit b 16
        #new_positional_embedding = nn.Parameter(scale* torch.randn(num_patches, width))
       # total_positional_embedding = torch.cat([self.positional_embedding, self.new_positional_embedding], dim=0)
        x = x + self.positional_embedding.to(x.dtype)
       # print("shape of x after duplicated positional embedding", x.shape)
       # x = x + self.positional_embedding.to(x.dtype) # duplicated positional embeddings
        #print("shape of x after positional embedding", x.shape)
        ...
        if self.VPT_shallow:
            ...
            assert self.prompt_till_layer_visual == 0
       # print("shape of x after visual context", x.shape)

        # Normal code as before
        x = self.ln_pre(x)
      #  print("shape of x after layer norm", x.shape)
